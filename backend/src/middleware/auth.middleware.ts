...
export const authenticateToken = async (
  ...
  try {
    ...
    console.log('🔍 AUTH DEBUG: Error in authentication:', (error as Error).message);
...
export const requireRole = (roles: string[]) => {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    const userRoles = req.user.roles.map((userRole: any) => userRole.role.name);
...
export const requirePermission = (permission: string) => {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    const userPermissions = req.user.roles.flatMap((userRole: any) => userRole.role.permissions);
...
export const requireMember = requireRole(['admin', 'moderator', 'member']);
